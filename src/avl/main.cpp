...
/** \mainpage
 *
 *  This documention describes code implementing a shared memory parallelization technique for the plane sweep algorithm.  
 *  The implementation uses openMP for parallelization, and shows speedup roughly equal to half the number of cores 
 *  available on a system.  
 *
 *  This code includes two versions.  One is a traditional plane sweep using an AVL tree for the active list and event queue.  
 *  The second implementation is identical except that it uses a vector for the AVL tree and event queue.  The vector version
 *  has time complexity \f$O(n^2)\f$.  The AVL version has time complexity \f$O((n \lg n) + (k \lg k))\f$ where \f$k\f$ is the number of intersections.
 *
